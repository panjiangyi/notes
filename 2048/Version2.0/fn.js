...
newGame.addEventListener("click", newGames, false);
...
function touch() {
  switch (arguments[0].type) {
    case "touchmove":
      ...
      if (
        arguments[0].touches[0].clientX >= X[0] &&
        arguments[0].touches[0].clientX <= X[1] &&
        arguments[0].touches[0].clientY >= Y[0] &&
        arguments[0].touches[0].clientY < Y[1]
      ) {
        arguments[0].preventDefault();
      }
      break;
    case "touchstart":
      start = [
        arguments[0].touches[0].clientX,
        arguments[0].touches[0].clientY,
      ];
      break;
    case "touchend":
      end = [
        arguments[0].changedTouches[0].clientX,
        arguments[0].changedTouches[0].clientY,
      ];
      var point = {
        X: end[0] - start[0],
        Y: end[1] - start[1],
      ...
      if (point.X > 0 && point.Y >= 0 && point.Y / point.X <= 1) {
        ...
        born();
        colorChange();
      } else if (point.X > 0 && point.Y > 0 && point.Y / point.X > 1) {
        Cdown();
        mapToBoard();
        born();
        colorChange();
